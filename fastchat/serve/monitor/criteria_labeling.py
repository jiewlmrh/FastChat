...
ENDPOINT_INFO = {"model_name":"META-LLAMA/LLAMA-3-70B-CHAT-HF",
                 ...
                 "parallel":8,
                 "temperature":0.0,
                 "max_token":512,} # Modify this

TAGS = {1:"specificity", 2:"domain_knowledge", 3:"complexity", 4:"problem_solving", 5:"creativity", 6:"technical_accuracy", 7:"real_world"}
...
def get_answer(
    question: dict, max_tokens: int, temperature: float, answer_file: str, api_dict: dict
):
    conv = []
...
if __name__ == "__main__":
    ...
    if args.cache_file:
        ...
        not_labeled = input_data[~input_data["question_id"].isin(cache_data["question_id"])]
    else:
    ...
    not_labeled["prompt"] = not_labeled.conversation_a.map(lambda convo: "\n".join([convo[i]["content"] for i in range(0, len(convo), 2)]))
